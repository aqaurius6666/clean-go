...
func newPost(db *gorm.DB, opts ...gen.DOOption) post {
	...
	_post.postDo.UseModel(&entities.Post{})
	...
	_post.CreatorID = field.NewString(tableName, "creator_id")
	_post.Creator = postBelongsToCreator{
...
func (p *post) updateTableName(table string) *post {
	...
	p.CreatorID = field.NewString(table, "creator_id")
...
type postBelongsToCreator struct {
	db *gorm.DB

	field.RelationField
}

func (a postBelongsToCreator) Where(conds ...field.Expr) *postBelongsToCreator {
	if len(conds) == 0 {
		return &a
	}
...
func (a postBelongsToCreator) WithContext(ctx context.Context) *postBelongsToCreator {
	a.db = a.db.WithContext(ctx)
	return &a
}

func (a postBelongsToCreator) Model(m *entities.Post) *postBelongsToCreatorTx {
	return &postBelongsToCreatorTx{a.db.Model(m).Association(a.Name())}
}

type postBelongsToCreatorTx struct{ tx *gorm.Association }

func (a postBelongsToCreatorTx) Find() (result *entities.User, err error) {
	return result, a.tx.Find(&result)
}

func (a postBelongsToCreatorTx) Append(values ...*entities.User) (err error) {
...
func (a postBelongsToCreatorTx) Replace(values ...*entities.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a postBelongsToCreatorTx) Delete(values ...*entities.User) (err error) {
	targetValues := make([]interface{}, len(values))
	for i, v := range values {
		targetValues[i] = v
	}
...
func (a postBelongsToCreatorTx) Clear() error {
	return a.tx.Clear()
}

func (a postBelongsToCreatorTx) Count() int64 {
	return a.tx.Count()
}
...
type IPostDo interface {
	...
	Unscoped() IPostDo
	Create(values ...*entities.Post) error
	CreateInBatches(values []*entities.Post, batchSize int) error
	Save(values ...*entities.Post) error
	First() (*entities.Post, error)
	Take() (*entities.Post, error)
	Last() (*entities.Post, error)
	Find() ([]*entities.Post, error)
	FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*entities.Post, err error)
	FindInBatches(result *[]*entities.Post, batchSize int, fc func(tx gen.Dao, batch int) error) error
	Pluck(column field.Expr, dest interface{}) error
	Delete(...*entities.Post) (info gen.ResultInfo, err error)
	...
	FirstOrInit() (*entities.Post, error)
	FirstOrCreate() (*entities.Post, error)
	FindByPage(offset int, limit int) (result []*entities.Post, count int64, err error)
...
func (p postDo) Create(values ...*entities.Post) error {
	if len(values) == 0 {
		return nil
	}
...
func (p postDo) CreateInBatches(values []*entities.Post, batchSize int) error {
	return p.DO.CreateInBatches(values, batchSize)
}

// Save : !!! underlying implementation is different with GORM
// The method is equivalent to executing the statement: db.Clauses(clause.OnConflict{UpdateAll: true}).Create(values)
func (p postDo) Save(values ...*entities.Post) error {
	if len(values) == 0 {
		return nil
	}
...
func (p postDo) First() (*entities.Post, error) {
	if result, err := p.DO.First(); err != nil {
		return nil, err
	} else {
		return result.(*entities.Post), nil
	}
}

func (p postDo) Take() (*entities.Post, error) {
...
func (p postDo) Last() (*entities.Post, error) {
...
func (p postDo) Find() ([]*entities.Post, error) {
	result, err := p.DO.Find()
	return result.([]*entities.Post), err
}

func (p postDo) FindInBatch(batchSize int, fc func(tx gen.Dao, batch int) error) (results []*entities.Post, err error) {
	buf := make([]*entities.Post, 0, batchSize)
...
func (p postDo) FindInBatches(result *[]*entities.Post, batchSize int, fc func(tx gen.Dao, batch int) error) error {
...
func (p postDo) FirstOrInit() (*entities.Post, error) {
...
func (p postDo) FirstOrCreate() (*entities.Post, error) {
...
func (p postDo) FindByPage(offset int, limit int) (result []*entities.Post, count int64, err error) {
...
func (p postDo) Delete(models ...*entities.Post) (result gen.ResultInfo, err error) {
